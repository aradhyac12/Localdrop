  ...
  try {
    for (let fi = 0; fi < files.length; fi++) {
      ...
      const pump = async (chunk) => {
        // Backpressure gate — stall the pump if the send buffer is too full.
        // CRITICAL: Race against channel close/error events. Without this race,
        // if the peer disconnects while bufferedAmount > HIGH_WATER the Promise
        // waits for 'bufferedamountlow' that will never fire — infinite hang.
        if (channel.bufferedAmount > HIGH_WATER) {
          await new Promise((resolve, reject) => {
            ...
            const onFail = (e) => {
              ...
              reject(new Error(
                e?.type === 'close'
                  ? 'Peer disconnected while buffer was full'
                  : 'DataChannel error during backpressure wait'
              ));
            };
            const cleanup = () => {
            ...
            channel.addEventListener('close',             onFail, { once: true });
            channel.addEventListener('error',             onFail, { once: true });
          });
        }
